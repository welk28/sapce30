...
<div id="cuerpo">
	...
	<section id="seccion">
        ...
        <div id="registros" >
        	<h2 class="">Lista de horarios de alta en el semestre actual 
        ...
        <table>
            ...
            <?php
				if($matricula)
				{
					...
					$horario="select g.idg, h.idh, h.idmat,p.sem, h.idoc, p.idcar, d.nomdoc, m.nommat, h.cupo from encarre as e, hgrupo as g, horario as h, posee as p, docente as d, materias as m where e.idg=g.idg and e.idcar=p.idcar and g.idh=h.idh and p.idmat=m.idmat and h.idmat=m.idmat and d.idoc=h.idoc and p.idmat=h.idmat and p.idcar='$ad->idcar' and h.periodo='$periodo' order by g.idg, p.sem;";
					...
					while($h=mysql_fetch_object($hora))
					{
						...
						{					
							...
							<tr>
								...
								<td>$h->nommat / $h->cupo<br> ** $h->nomdoc</td>";
								...
								else
								{
									...
									//if(($ad->status >1)&&($n>=60))
									if($h->idmat=='RESIDENCIA')
									{

										echo"<td><a href='validaliga.php?idh=$h->idh&idmat=$h->idmat&matricula=$matricula&idcar=$ad->idcar' target='_self'>Agre  </a></td>";
										//echo"<td><a href='cursar.php?idh=$h->idh&idmat=$h->idmat&matricula=$matricula&idcar=$ad->idcar' target='_self'>Agre  </a></td>";
									}
									else
									{
										if($n<$h->cupo)
				...
				else
				{
					...
					while($c=mysql_fetch_object($carre))
					{
						...
						{
							$q++;		
							//<a href='desag.php?idg=$h->idg&idh=$h->idh' target='_self' title='Desasignar a este grupo'>			</a>
							echo"
								<tr>
									<td align='center'>$q </td>
									<td>$h->idg</a></td>
									...
									$tic="select distinct c.matricula from alumnos as a, cursa as c, horario as h where a.matricula=c.matricula and h.idh=c.idh and h.idh='$h->idh' and a.idcar='ITIC-2010-225';";
									$ti=mysql_query($tic,$conexion);
									$tn=mysql_num_rows($ti);

									$log="select distinct c.matricula from alumnos as a, cursa as c, horario as h where a.matricula=c.matricula and h.idh=c.idh and h.idh='$h->idh' and a.idcar='ILOG-2009-202';";
									$lo=mysql_query($log,$conexion);
									$ln=mysql_num_rows($lo);

									$adm="select distinct c.matricula from alumnos as a, cursa as c, horario as h where a.matricula=c.matricula and h.idh=c.idh and h.idh='$h->idh' and a.idcar='IADM-2010-213';";
									$ad=mysql_query($adm,$conexion);
									$an=mysql_num_rows($ad);
									echo"
									<td>$tn</td>
									<td>$ln</td>
									<td>$an</td>
									...
									echo"$n</td>";
									if(($ses==2)||($ses==5))
									echo"<td><a href='agregadia.php?idh=$h->idh' target='_self' title='Asignar hora/dia'>Modif</a></td>";
								echo"</tr>
							";
						}
					}
