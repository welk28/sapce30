...
<div id="cuerpo">
	...
	while($ca=mysql_fetch_object($carre))
	{
		$prom=0;
		$can=0;
		$cantidad="select distinct c.matricula from cursa as c, horario as h, alumnos as a where a.matricula=c.matricula and h.idh=c.idh and h.periodo='$periodo' and a.idcar='$ca->idcar';";
		$cant=mysql_query($cantidad,$conexion);
		$can=mysql_num_rows($cant);
    echo"
	...
	<table>
		...
		$alumnos="select distinct c.matricula, a.app, a.apm, a.nom, a.status from horario as h, alumnos as a, cursa as c where h.idh=c.idh and c.matricula=a.matricula and h.periodo='$periodo' and (c.opor='2' or c.opor='3' or c.opor='5') and a.idcar='$ca->idcar' order by a.matricula desc;";
    	...
    	$prom=($x*100)/$can;
    	echo"<br>Total de alumnos: $can <br> Irregulares: $x <br> PROMEDIO DE ALUMNOS REPROBADOS en $ca->idcar: $prom";
    echo"</table>
